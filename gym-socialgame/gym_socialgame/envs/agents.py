...
class Consumption:
	...
	def __init__(self, name, curtailment_factor, shift_factor, foresight_time, time_preference, demand_productivity_factor_by_time,
					contiguous_preference, total_demand, max_demand):
		# name/identifier of consumption
		self.name:str = name
		# curtailment_factor, def: scalar value representing preference to not curtail this consumption
		self.curtailment_factor:float = curtailment_factor
		assert curtailment_factor >= 0
		# shift_factor, def: scalar value representing preference to not shift this consumption
		self.shift_factor:float = curtailment_factor
		assert shift_factor >= 0
		# foresight_time, def: amount of time in the future in which a unit of demand may be shifted


######################################################
class ActivtyConsumer:
	'''
	props:
		- foresight
		- activities
	...
	def get_response(durationOfConsumption):
		activity_buckets:Sequence[Activity] = []
...
class Activity:
	...
	props:
		- demand units
		- activities | children activity objects ? consider if required for good design
		- blocking | activites tendency to block other activites ? vector value, blocks/encourages all non-orthogonal vectors
		- threshold | threshold at which this activity is completed
		- price effect
	'''
	# props:
	# demand units
	# activities - children activity objects ? consider if good design
	# blocking - activites tendency to block other activites
...
class DemandUnit:
	'''
	props:
		- activites | all parent activities (multiple activites may connect to the same demand unit)
		...
	'''
	
	def __init__(self, name, curtailment_anti_preference, shift_anti_preference, time_preference, anchour_time, foresight, 
				total_demand, contiguous_preferences, demand_productivity_factor_by_time):
		# name, def: name/identifier of consumption
		self.name : str = name
		# curtailment_anti_preference, def: scalar value representing preference to not curtail this unit of consumption
		# def continued: 0 represents maximum willinginess to curtail demand
		# def continued: 1 represents minimum willinginess to curtail demand, eg cannot curtail demand
		assert 0 <= curtailment_anti_preference <= 1
		self.curtailment_anti_preference : float = curtailment_anti_preference
		# shift_anti_preference, def: scalar value representing preference to not shift this unit of consumption
		# def continued: 0 represents maximum willinginess to shift demand
		# def continued: 1 represents minimum willinginess to shift demand, eg cannot shift demand
		assert 0 <= shift_anti_preference <= 1
		self.shift_anti_preference : float = shift_anti_preference
		# time_preference, def: Series of preferences for consumption of demand unit
		# def continued: 0 represents minimum preference, 1 represents maximum preference
		# Additional: first element at timestamp given by (anchour_time - foresight)
		assert all(0 <= preference <= 1 for preference in time_preference)
		self.time_preference : pd.Series = time_preference
		####
		self.start_time : pd.Timestamp = anchour_time - foresight

		self.total_demand : float = total_demand

		# contiguous_preferences, def: Series of preferences for having demand in x units
		# def continued: 0 represents minimum preference, 1 represents maximum preference
		assert pd.Series.sum(contiguous_preferences) == 1
		self.contiguous_preferences: pd.Series = contiguous_preferences

		# demand_productivity_factor_by_time, the productivity of a task towards demand by time step
		self.demand_productivity_factor_by_time: pd.Series = demand_productivity_factor_by_time

	def getCost():
		pass

	def optimize(time_prices:pd.Series) -> pd.Series:
		'''Get the demand series for this demand unit 
		'''
		pass
